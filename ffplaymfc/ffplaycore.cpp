...
#define FF_VIDEOSIZE	 (SDL_USEREVENT + 100)
...
static void calculate_display_rect(SDL_Rect *rect, int scr_xleft, int scr_ytop, int scr_width, int scr_height, VideoPicture *vp)
{
	...
	height = scr_height;
	width = ((int)myrint(height * aspect_ratio)) & ~1;
	if (width > scr_width) {
		width = scr_width;
		height = ((int)myrint(width / aspect_ratio)) & ~1;
	}
	x = (scr_width - width) / 2;
...
static void video_image_display(VideoState *is)
{
	...
	if (vp->bmp) {
		...
		g_disp.DisplayTexture(vp->bmp, rect);
	}
}
...
static void alloc_picture(VideoState *is)
{
	...
	TRACE(TEXT("bmp : %d %d %d \n"), vp->width, vp->height, is->pictq_windex);
	if (!vp->bmp) {
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts1, int64_t pos)
{
	...
	* pts1 is the dts of the pkt / pts of the frame */
	...
	if (vp->bmp) {
		g_disp.UpdateTexture(vp->bmp, (AVPixelFormat)src_frame->format, src_frame->data, src_frame->linesize);
...
static int stream_component_open(VideoState *is, int stream_index)
{
	...
	case AVMEDIA_TYPE_VIDEO:
		...
		//视频线程
		is->video_tid = SDL_CreateThread(video_thread, "video-tid", is);
		break;
	case AVMEDIA_TYPE_SUBTITLE:
		...
		is->subtitle_tid = SDL_CreateThread(subtitle_thread, "subtitle-tid", is);
		break;
	default:
		break;
	}
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
	...
	//解码线程
	is->read_tid     = SDL_CreateThread(read_thread, "read-tid", is);
	if (!is->read_tid) {
		av_free(is);
		return NULL;
	}
...
{
	g_disp.FillWindow(is->xleft, is->ytop, is->width, is->height, 0x0, 0x0, 0x0);
	g_disp.UpdateRect(is->xleft, is->ytop, is->width, is->height);
}
